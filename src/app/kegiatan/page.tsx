...
export default function KegiatanPage() {
  ...
  const handleDelete = () => {
    ...
    toast({ title: "Terhapus", description: "Laporan kegiatan telah dihapus." })
    ...
    <div className="flex flex-col gap-6 p-4 md:p-8 overflow-x-hidden">
      <header className="flex items-center gap-4">
        ...
        <div>
            ...
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Dokumentasi Digital Desa</p>
        </div>
      </header>
        ...
        <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
          <DialogTrigger asChild>
            <Button className="w-full h-14 gap-3 text-base font-black uppercase shadow-lg bg-primary hover:bg-primary/90 rounded-2xl">
              <Plus className="h-6 w-6" /> Upload Laporan Baru
            </Button>
          </DialogTrigger>
          <DialogContent className="w-[95vw] sm:max-w-[500px] max-h-[95vh] overflow-y-auto p-4 sm:p-6 rounded-[2.5rem] border shadow-2xl">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs font-bold uppercase text-muted-foreground">Dokumentasikan kegiatan Anda.</DialogDescription>
            </DialogHeader>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.length > 0 ? (
            ...
            <div className="col-span-full py-20 text-center border-2 border-dashed rounded-[2.5rem] bg-muted/5 border-muted/50">
              <p className="font-black text-muted-foreground uppercase text-sm">Belum Ada Laporan</p>
            </div>
          )}
        </div>
      )}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="w-[95vw] sm:max-w-[500px] max-h-[95vh] overflow-y-auto p-4 sm:p-6 rounded-[2.5rem] border shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black text-primary uppercase">Sunting Laporan</DialogTitle>

...
export async function getNextSequenceNumber(db: Firestore, kategori: string, classification: string = "000"): Promise<string> {
  ...
  // Namun User ingin No Surat Tugas SPPD masuk ke Buku Agenda kategori 'surat_keluar'
  // Jadi kita harus cek nomor terakhir di kategori tersebut.
  ...
  const q = query(
    ...
    limit(20) // Ambil beberapa untuk memastikan tahun yang benar
  );
  ...
  if (!snapshot.empty) {
    // Cari dokumen terbaru di tahun berjalan
    const latestDoc = snapshot.docs.find(d => (d.data().createdAt || "").substring(0, 4) === currentYear);
    
    if (latestDoc) {
      const lastNomor = latestDoc.data().nomor || "";
      // Ekstrak angka 3 digit dari nomor (format: .../XXX/...)
  ...
  if (kategori === "surat_tugas_sppd") {
    // Gunakan klasifikasi Kepegawaian (800) untuk Surat Tugas

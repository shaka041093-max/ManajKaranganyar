...
import { ImagePlus, Loader2, FileText, Upload, Calendar as CalendarIcon, RefreshCw, Printer, FileCheck, Sparkles, BookOpen, AlertCircle, ChevronRight, CheckCircle, Save } from "lucide-react"
...
import { generateNotulenPDF, generateBASTPDF, generateDokumentasiPDF } from "@/lib/pdf-utils"
...
export function KegiatanUpload({ onSuccess, initialData }: { onSuccess?: () => void, initialData?: any }) {
  ...
  }, [selectedCalendarDate, handleSync, initialData])
  ...
  const handleSelectAgenda = (agenda: AgendaItem) => {
    ...
    setActiveTab("manual")
    toast({ title: "Agenda Terpilih", description: `Data telah terisi otomatis sesuai tipe agenda.` })
  }
  ...
  const handlePreviewCategoryDok = async (category: 'kegiatan' | 'atk' | 'konsumsi', urls: string[]) => {
    ...
    try {
        ...
        const pdfBlob = await generateDokumentasiPDF(values, title, files, userData?.logoBase64);
        const url = URL.createObjectURL(pdfBlob);
        window.open(url, "_blank");
    } catch (e) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (cloudinaryUrls.kegiatan.length > 0) {
        ...
        const blob = await generateDokumentasiPDF(values, "DOKUMENTASI KEGIATAN", files, userData?.logoBase64);
      ...
      if (watchActivityType === "Internal" && cloudinaryUrls.atk.length > 0) {
        ...
        const blob = await generateDokumentasiPDF(values, "DOKUMENTASI ATK", files, userData?.logoBase64);
      ...
      if (watchActivityType === "Internal" && cloudinaryUrls.konsumsi.length > 0) {
        ...
        const blob = await generateDokumentasiPDF(values, "DOKUMENTASI KONSUMSI", files, userData?.logoBase64);
      ...
      if (initialData?.id) {
        const docRef = doc(db, "users", user.uid, "kegiatans", initialData.id);
        updateDocumentNonBlocking(docRef, docData);
        toast({ title: "Laporan Diperbarui!", description: `Data berhasil diupdate dan dokumen PDF baru telah diunggah ke Drive.` });
      } else {
        const kegiatanRef = collection(db, "users", user.uid, "kegiatans");
        addDocumentNonBlocking(kegiatanRef, docData);
        toast({ title: "Berhasil!", description: `Data tersimpan di Firestore & Seluruh dokumen PDF diarsipkan ke Drive.` });
      }
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1 whitespace-normal break-words font-medium">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pb-4">
              ...
              <Button type="submit" className="w-full h-14 text-base font-black uppercase shadow-lg shadow-primary/20 bg-primary hover:bg-primary/90" disabled={isUploading}>
                {isUploading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> {initialData ? 'Memperbarui...' : 'Mengarsipkan...'}</> : initialData ? <><Save className="mr-2 h-5 w-5" /> Perbarui Laporan</> : <><Upload className="mr-2 h-5 w-5" /> Simpan Ke Drive & Firestore</>}
              </Button>
            </form>
          </Form>
        </TabsContent>
